...
//	USI_I2C_Slave_State = USI_SLAVE_CHECK_ADDRESS;
...

	{
        ...
        // Clear the Start Condition Flag and set the internal address to 0
        USISR = (1 << USISIF) | (1 << USIOIF) | (1 << USIPF) | (1 << USIDC);
        usi_i2c_slave_internal_address = 0;

        // Set SDA as input for checking the start condition
        USI_SET_SDA_INPUT();
...
char DataAvailable() {
    ...
    return i2cDataIndex < sizeof(i2c_registers);
}
...
//			if((USIDR == 0) || ((USIDR >> 1) == usi_i2c_slave_address))
			if(((USIDR >> 1) == usi_i2c_slave_address))
...
//			USI_Slave_internal_address++;
		...
		case USI_SLAVE_RECV_DATA_ACK_SEND:
			...
			{
				i2c_registers[USI_Slave_internal_address] = USIDR;
			}
