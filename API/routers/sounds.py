...
async def get_sound(sound_id: int = Query()):
    with get_session() as session:
...
async def add_sound(input_sound: SoundCreate):
    with get_session() as session:
...
async def update_sound(input_sound: SoundUpdate, sound_id: int = Query()):
    with get_session() as session:
        sound = session.get(Sounds, sound_id)
        if (not sound):
            raise HTTPException(status_code=404, detail="Sound not found")
...
async def remove_sound(sound_id: int = Query()):
    with get_session() as session:
        sound = session.get(Sounds, sound_id)
        if (not sound):
            raise HTTPException(status_code=404, detail="Sound not found")
