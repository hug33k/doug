...
async def get_buttons(session: Session = Depends(get_session)):
	buttons = session.exec(select(Buttons)).all()
	return buttons


@router.get("/{button_id}", response_model=Union[ButtonRead, None])
async def get_button(button_id: int, session: Session = Depends(get_session)):
...
async def add_button(input_button: ButtonCreate, session: Session = Depends(get_session)):
...
async def update_button(input_button: ButtonUpdate,
						button_id: int,
...
async def remove_button(button_id: int, session: Session = Depends(get_session)):
	button = session.get(Buttons, button_id)
	if not button:
		raise HTTPException(status_code=404, detail="Button not found")
